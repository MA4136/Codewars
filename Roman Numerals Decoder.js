...
function solution(roman) {
    ...
    return roman.match(/CM|CD|XC|XL|IX|IV|\w/g).reduce((acc, roman) => acc + numerals[roman], 0)
}
...
function solution(roman) {
    ...
    return [...roman].map(el => numerals[el]).reduce((acc, curr) => acc < curr ? curr - acc : acc + curr)
}
